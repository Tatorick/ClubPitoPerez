  ...
  return (
    <section className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
      <div className="px-6 py-4 border-b border-gray-100 flex items-center gap-3">
        <div className="w-9 h-9 rounded-xl bg-[#001f3f]/10 flex items-center justify-center shrink-0">
          <span className="material-symbols-outlined text-[20px] text-[#001f3f]">{icon}</span>
        </div>
        <div className="flex-1">
          <h3 className="font-bold text-gray-800 text-sm">{title}</h3>
          {badge && (
            <span className={`inline-block mt-0.5 px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider
              ${badge === 'Configurado' ? 'bg-green-100 text-green-700' :
                badge === 'Pruebas' ? 'bg-amber-100 text-amber-700' :
                badge === 'Producción' ? 'bg-blue-100 text-blue-700' :
...
export default function ConfigView() {
  ...
  const handleChange = (e) => {
    ...
    // Cambiar URL base automáticamente según el ambiente
  ...
  // ── Probar conexión con AutorizadorEC ───────────────────────────────────────
  ...
  // ── Verificar si la config está completa para facturar ─────────────────────
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      {saveSuccess && (
        <div className="flex items-center gap-2 p-4 rounded-xl bg-green-500 text-white text-sm font-semibold animate-[fadeIn_0.2s_ease-out]">
      ...
      {/* ═══ SECCIÓN 1: DATOS DEL CLUB EMISOR ══════════════════════════════════ */}
      <ConfigSection icon="business" title="Datos del Club Emisor"
        badge={form.ruc.length === 13 && form.razon_social ? 'Configurado' : 'Incompleto'}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <ConfigField label="Nombre Comercial"
            hint="Si está vacío, se usa la razón social.">
          ...
          <div className="md:col-span-2">
            <ConfigField label="Correo Electrónico del Club"
              ...
              <input name="email_club" type="email" value={form.email_club}
                onChange={handleChange} placeholder="contacto@clubpitofirez.com"
                className={inputCls} />
            </ConfigField>
          </div>
        </div>
      ...
      {/* ═══ SECCIÓN 2: ESTABLECIMIENTOS SRI ═══════════════════════════════════ */}
      <ConfigSection icon="store" title="Establecimiento y Punto de Emisión SRI"
        badge={`${form.cod_establecimiento}-${form.cod_punto_emision}`}>
      ...
      {/* ═══ SECCIÓN 3: AUTORIZADOREC ═══════════════════════════════════════════ */}
      <ConfigSection icon="api" title="Integración AutorizadorEC"
        badge={form.autorizadorec_api_key ? form.autorizadorec_ambiente === 'produccion' ? 'Producción' : 'Pruebas' : 'Sin configurar'}>
        ...
        {form.autorizadorec_ambiente === 'produccion' && (
          <div className="flex items-start gap-2 p-3 rounded-xl bg-blue-50 border border-blue-300 text-blue-800 text-xs mb-4">
            ...
            <p><strong>Modo Producción activo.</strong> Las facturas emitidas serán documentos oficiales válidos ante el SRI. Asegúrate de que tus datos estén correctos.</p>
          </div>
        )}

        {/* Instrucciones si no tiene cuenta */}
        ...
        <div className="grid grid-cols-1 gap-4">
          ...
          {/* URL del Webhook (para copiar y pegar en AutorizadorEC) */}
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5">
          ...
          <div className="space-y-2">
            <p className="text-xs text-gray-600">
              El logo aparece en los comprobantes RIDE generados por AutorizadorEC (sujeto a soporte del proveedor). Formato PNG o JPG, máximo 2 MB.
            </p>
      ...
      <div className="flex items-center justify-between pt-2 pb-8">
        <p className="text-xs text-gray-400">
          Los cambios en la configuración aplican a las <strong>próximas facturas emitidas</strong>. Las facturas ya emitidas no se modifican.
        </p>
        <button type="button" onClick={handleSave} disabled={saving}
          className="flex items-center gap-2 px-6 py-3 rounded-xl bg-[#001f3f] text-white text-sm font-bold hover:bg-blue-900 transition-colors shadow-md disabled:opacity-60 disabled:cursor-not-allowed">
          {saving
            ? <><span className="material-symbols-outlined text-[18px] animate-spin">progress_activity</span> Guardando...</>
            : <><span className="material-symbols-outlined text-[18px]">save</span> Guardar Configuración</>
          }
        </button>
      </div>
